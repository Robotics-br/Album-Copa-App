...
export default function TeamSelector({ selectedTeamId, onSelectTeam }: TeamSelectorProps) {
  ...
  const selectedTeam = useMemo(() => {
    return selectedTeamId ? teams.find((s) => s.id === selectedTeamId) || null : null;
  }, [selectedTeamId]);
  ...
  const clearFilter = (e: any) => {
    e.stopPropagation();
    selectionTap();
    onSelectTeam(''); // events.tsx expects empty string instead of null for clearing usually, but the type allows string for now. The events.tsx state is string ''. Wait, in SectionSelector it's null. I'll use null or empty string. The type from events.tsx says `setTeamId('')`, so I should probably send empty string or handle it properly. Wait, I will use `''` for clear if the prop is used like that. I'll stick to `setTeamId('')` equivalent when we integrate. Actually, I'll pass `''` from the caller. Let's just use empty string or null interchangeably and I will cast it in the callback.
  };

  const getName = (id: string) => {
    return i18n_t(`teams.${id}`);
  };
          ...
          <View className="flex-1 flex-row items-center">
            ...
            <View className="flex-1">
              <Text className="text-[12px] font-medium uppercase tracking-tight text-text-secondary">
                {i18n_t('games.filters.team')}
              </Text>
              <Text className="text-[15px] font-bold text-text">
                {selectedTeam ? getName(selectedTeam.id) : i18n_t('filters.all')}
              </Text>
            </View>
          </View>
              ...
              <FlashList
                ...
                ListHeaderComponent={() => (
                  <TouchableOpacity
                    ...
                    <Text
                      className={`flex-1 text-[17px] ${!selectedTeamId ? 'font-bold text-primary' : 'font-medium text-text'}`}>
                      {i18n_t('filters.all')}
                    </Text>
                    {!selectedTeamId && <View className="h-2.5 w-2.5 rounded-full bg-primary" />}
                  </TouchableOpacity>
                )}
                renderItem={({ item }) => {
                  const isActive = selectedTeamId === item.id;
